...
어떠한 프로세스가 들어오면 위와같은방식으로 실행된다.

이 방식은 Single Ready Queue -> Several separate Queue 가 되었다고 볼 수 있고
...
Feedback이란, 뒤에서 다시 앞으로 오는 ! 의미를 가지고있다. 그래서 복수개의 Queue(각 정책이 다른)을 가지고있을 떄 한 큐에 머물러있지 않고 다른 Queue로의 점진적인 이동을 하는데, 아래와같이 위아래로 슉슉 이동한다.

-   모든 프로세스는 하나의 입구로 진입하여
-   너무 많은 CPU time 사용시 다른 Queue로이동
-   starving 상태(너무 오래기다림)우려될 경우 우선순위 높은 Queue로이동

예를들어, Several Separate Queue를 사용하여 어떠한 `BatchReadyQueue`에서 기다리고있다가, 서비스를 받고 나가고,,,를 반복해도 그 해당 프로세스가 작업이 끝나지 않으면 (잘려서 처리되는경우도있으니) -> `InteractiveReadyQueue` 에서 처리,, 그래도 작업이 오랫동안 잘 안끝나면 -> ` SystemReadyQueue``BatchReadyQueue ` ... 이렇게 큐를 옮겨가며(정책 변경) 프로세스를 처리시키는 것을 말한다.
...
그럴 경우가 우리 고성능의 프로그램을 저성능의 컴퓨터에서 사용할 때 많이 발생하는 이유와 관련이 있는가?
```

💭 그렇다면 윈도우와 리눅스는 어떠한 방식을 사용할까?
...
-   프로세스 생성(system call)
    ...
    -   `exit()` : 해당ㅇ 프로세스가 가졌던 모든 자원은 OS에게 반환
