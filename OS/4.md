...
우선, 프로세스를 어떠한 방식으로 종료시킬지에 대해 두가지로 구분이된다.
...
예를들어, Several Separate Queue를 사용하여 어떠한 `BatchReadyQueue`에서 기다리고있다가, 서비스를 받고 나가고,,,를 반복해도 그 해당 프로세스가 작업이 끝나지 않으면 (잘려서 처리되는경우도있으니) `starving`상태라고 판단하여 -> `InteractiveReadyQueue` 에서 처리,, 그래도 작업이 오랫동안 잘 안끝나면 -> ` SystemReadyQueue``BatchReadyQueue ` ... 이렇게 큐를 옮겨가며(정책 변경) 프로세스를 처리시키는 것을 말한다.
